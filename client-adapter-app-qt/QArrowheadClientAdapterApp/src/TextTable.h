...
class TextTable {
    ...
    enum class FormattedTable { FIRST_COLUMN, FIRST_ROW, NONE } format_type;
    ...
    void setFormat(FormattedTable format_type, FormattedText format){
...
{
    std::string vertical = std::string(1, table.vertical());
#ifndef CREATE_DOCUMENTATION
    ...
    for ( auto rowIterator = table.rows().begin(); rowIterator != table.rows().end(); ++ rowIterator ) {
        ...
        stream << FormattedText::formatText(table.format, vertical);
        for ( unsigned i = 0; i < row.size(); ++i ) {
            ...
            stream << (   (rowIterator == table.rows().begin() && table.format_type == TextTable::FormattedTable::FIRST_ROW )||
                          (i == 0 && table.format_type == TextTable::FormattedTable::FIRST_COLUMN) ? table.format : FormattedText{FormattedText::reset})
                   << std::setw( table.width( i ) + table.correctDistance(row[ i ])) << alignment <<
                      row[ i ];
            stream << FormattedText::formatText(table.format, vertical);
        }
        stream << "\n";
    ...
    stream << FormattedText{FormattedText::reset};
